...
class pension_cycle(osv.osv):
            ...
            for records in res_partner_browse:
                ...
                for line in records.history_transaction:
                    if line.status == 'pending':
                        ...
                        date_reference = line.date_reference
                        status = 'Pending'
                pending_list.append((0,0,{'application_no':application_no,'date_reference':date_reference,'name_appli':name_appli,'date_year':year,'date_month':for_month,'sanctioned_amount':amount,'status':status,'full_reconcile':False}))
            return pending_list
                    ...
                    if line_in.date_reference: 
                        if datas == line_in.date_reference:  # Matching Date field with sorted date
                            ...
                            if amount_single <= balance_amount:
                                ...
                                full_reconcile = True
                                return_list.append((0,0,{'application_no':application_no,'name_appli':name_appli,'date_reference':date_reference,'date_year':date_year,'date_month':date_month,'sanctioned_amount':sanctioned_amount,'status':status,'full_reconcile':full_reconcile}))
            for datas in pending_year_list:
                for line_in in rec.pension_line_id:
                    application_no = line_in.application_no
                    name_appli = line_in.name_appli.id
                    ...
                    if line_in.date_reference:    
                        if datas != line_in.date_reference:  # Matching Date field with sorted date
                            return_list.append((0,0,{'application_no':application_no,'name_appli':name_appli,'date_reference':date_reference,'date_year':date_year,'date_month':date_month,'sanctioned_amount':sanctioned_amount,'status':status,'full_reconcile':full_reconcile}))
            self.write(cr,uid,ids,{'pension_line_id':delete_list})
    ...
    _columns = {
            ...
            'actual_amount':fields.float('Actual Required Amount'),
