...
class sale_inherit(osv.osv): 
    ...
    ####################################### UPDATED #################################################
    #################################################################################################
    ############################### PLEASE DONT ADD THIS CODE TO SERVER #############################
    ######################################## ON PROGRESS ############################################
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        if context is None:
            context = {}
        #if (not view_id) and (view_type=='form') and (view_type=='tree') and context and context.get('force_email', False):
        #    view_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'base', 'view_partner_simple_form')[1]
        res = super(sale_inherit, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type,context=context, toolbar=toolbar, submenu=submenu)
        doc = etree.XML(res['arch'])
        cust = 1
        if context.get('is_wakf', False):
            if context['is_wakf'] == '1':
                cust = 1
            elif context['is_wakf'] == '0':
                cust = 0
 ...
 #####################################################################################################################################
    ...
    def action_sanction(self, cr, uid, ids, context=None):
        ...
        if rec.head.name == "Pension":
            ...
            transaction_list.append((0,0,{'date_reference':date_sanction,'for_month':month,'year':year,'amount':price_unit,'status':'pending'}))
...
class pension_disease_history(osv.osv):
    ...
    _columns = {
            ...
            'date_reference':fields.date('Date'),
